...
export const UserDetailWidget = memo(
  ({ user, isLoading, isError, error, isOwnProfile = false }: UserDetailWidgetProps) => {
    ...
    // ── Success ──
    const handleToggleExtraCostPermission = async () => {
      if (!user || !canReceiveExtraCostPermission || !canManagePrivilegedToggles) return;

      const nextValue = !user.can_add_extra_cost;

      await apiRequest({
        request: () =>
          updateUser.mutateAsync({
            id: user.id,
            data: { can_add_extra_cost: nextValue },
          }),
    ...
    const handleToggleMarketAddOrder = async () => {
      if (!user || !isMarket || !canManagePrivilegedToggles) return;
      ...
      await apiRequest({
        ...
        successMessage: nextValue
          ...
          : t("marketAddOrderDisabled", {
              defaultValue: "Market uchun buyurtma qo'shish o'chirildi",
            }),
        errorMessage: t("editUserError"),
      });
    };
    ...
    const handleToggleMarketCancelledHandoverQr = async () => {
      ...
      await apiRequest({
        ...
        successMessage: nextValue
          ? t("cancelledHandoverQrEnabled", {
              defaultValue: "Bekor qilingan orderlarni topshirishda market QR talabi yoqildi",
            })
          : t("cancelledHandoverQrDisabled", {
              defaultValue: "Bekor qilingan orderlarni topshirishda market QR talabi o'chirildi",
            }),
        errorMessage: t("editUserError"),
      });
    };
          ...
          <div className="w-full min-w-0 flex-1 space-y-4">
            <UserInfoCards
              ...
              onToggleMarketAddOrder={
                isMarket && canManagePrivilegedToggles ? handleToggleMarketAddOrder : undefined
              }
              isMarketAddOrderPending={updateMarketAddOrder.isPending}
              onToggleMarketProof={
                isMarket && canManagePrivilegedToggles ? handleToggleMarketProof : undefined
              }
              isMarketProofPending={updateMarketExpenseProof.isPending}
              onToggleMarketCancelledHandoverQr={
                isMarket && canManagePrivilegedToggles
                  ? handleToggleMarketCancelledHandoverQr
                  : undefined
              }
              isMarketCancelledHandoverQrPending={updateMarketCancelledHandoverQr.isPending}
              onToggleExtraCostPermission={
                canReceiveExtraCostPermission && canManagePrivilegedToggles
                  ? handleToggleExtraCostPermission
                  : undefined
              }
