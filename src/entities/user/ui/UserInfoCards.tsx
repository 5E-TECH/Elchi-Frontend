...
const InfoChip = ({ icon: Icon, iconBg, iconColor, label, value, wide }: ChipProps) => (
  <div
    className={`group flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 transition-all hover:border-main/30 hover:shadow-sm dark:border-white/5 dark:bg-white/5 dark:hover:border-main/30 sm:p-4 ${wide ? "md:col-span-2" : ""}`}
  >
    <div
      className={`h-9 w-9 shrink-0 rounded-xl ${iconBg} flex items-center justify-center transition-transform group-hover:scale-110`}
    >
              ...
              <InfoChip
                ...
                value={
                  <span className="text-orange-500 font-bold">
                    {Number(user.tariff_home ?? 0).toLocaleString("uz-UZ")} so'm
                  </span>
                }
              />
              ...
              <InfoChip
                ...
                value={
                  <span className="text-amber-500 font-bold">
                    {Number(user.tariff_center ?? 0).toLocaleString("uz-UZ")} so'm
                  </span>
                }
              />
            </>
          ...
          {isMarket && (
            <>
              ...
              <InfoChip
                ...
                iconBg="bg-amber-50 dark:bg-amber-500/10"
                iconColor="text-amber-500"
                label={t("cancelledHandoverQrRequired", {
                  defaultValue: "Bekor qilinganlar uchun QR",
                })}
                value={
                  <ToggleButton
                    checked={user.cancelled_handover_qr_required !== false}
                    disabled={
                      !onToggleMarketCancelledHandoverQr || isMarketCancelledHandoverQrPending
                    }
                    label={t("cancelledHandoverQrRequired", {
                      defaultValue: "Bekor qilinganlar uchun QR",
                    })}
              ...
              {canManageProofSettings && (
                <>
                  ...
                  <InfoChip
                    ...
                    value={
                      <ToggleButton
                        ...
                        disabled={!onToggleMarketProof || isMarketProofPending}
                        label={t("cancelMediaProofRequired", {
                          defaultValue: "Bekor qilishda isbot",
                        })}
                  ...
                  <InfoChip
                    ...
                    iconBg="bg-emerald-50 dark:bg-emerald-500/10"
                    iconColor="text-emerald-500"
                    label={t("sellExtraCostProofRequired")}
                    value={
                      <ToggleButton
                        checked={proofConditions.includes("sell_extra_cost")}
                        disabled={!onToggleMarketProof || isMarketProofPending}
                        label={t("sellExtraCostProofRequired")}
                        onClick={() => onToggleMarketProof?.("sell_extra_cost")}
                      />
                    }
                  />
                  ...
                  <InfoChip
                    ...
                    iconBg="bg-rose-50 dark:bg-rose-500/10"
                    iconColor="text-rose-500"
                    label={t("cancelExtraCostProofRequired")}
                    value={
                      <ToggleButton
                        checked={proofConditions.includes("cancel_extra_cost")}
                        disabled={!onToggleMarketProof || isMarketProofPending}
                        label={t("cancelExtraCostProofRequired")}
                        onClick={() => onToggleMarketProof?.("cancel_extra_cost")}
                      />
                    }
                  />
                </>
