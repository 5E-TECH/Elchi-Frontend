...
export const useCashBox = () => {
  ...
  const getFinanceHistory = (params?: any) =>
    useQuery({
      ...
      queryFn: () =>
        api.get(`finance/history`, { params }).then((res) => res.data),
    });
  ...
  const getOperationTypes = () =>
    useQuery({
      ...
      queryFn: () => api.get("finance/operation-type").then((res) => res.data),
    });

  const getSourceTypes = () =>
    useQuery({
      queryKey: [cashbox, "source-types"],
      queryFn: () => api.get("finance/source-type").then((res) => res.data),
    });
