...
import { User, Clock, Hourglass, BadgeCheck, XCircle, Truck, PackageCheck, Sparkles, CircleDollarSign, CheckCircle2 } from "lucide-react";
...
const statusConfig: Record<Order_status, { icon: JSX.Element; className: string }> = {
  [Order_status.CREATED]: { icon: <Clock size={13} />, className: "bg-gray-100 text-gray-600 dark:bg-gray-700/50 dark:text-gray-300" },
  [Order_status.NEW]: { icon: <Sparkles size={13} />, className: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300" },
  [Order_status.RECEIVED]: { icon: <PackageCheck size={13} />, className: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300" },
  [Order_status.ON_THE_ROAD]: { icon: <Truck size={13} />, className: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300" },
  [Order_status.WAITING]: { icon: <Hourglass size={13} />, className: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300" },
  [Order_status.SOLD]: { icon: <BadgeCheck size={13} />, className: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300" },
  [Order_status.CANCELLED]: { icon: <XCircle size={13} />, className: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300" },
  [Order_status.PAID]: { icon: <CircleDollarSign size={13} />, className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" },
  [Order_status.PARTLY_PAID]: { icon: <CircleDollarSign size={13} />, className: "bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300" },
  [Order_status.CANCELLED_SENT]: { icon: <XCircle size={13} />, className: "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300" },
  [Order_status.CLOSED]: { icon: <CheckCircle2 size={13} />, className: "bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300" },
};
const fallbackStatus = { icon: <Clock size={13} />, className: "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" };
...
const PendingOrdersTable = ({ orders, loading, onDeliver, onCancel }: Props) => {
      ...
      render: (val) => {
        ...
        return (
          <span className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full w-fit ${cfg.className}`}>
            {cfg.icon}{val as string}
          </span>
        );
      },
    },
    {
      key: "total_price", label: "Narx", sortable: true,
      render: (val) => <span className="font-bold text-sm">{Number(val).toLocaleString("uz-UZ")}</span>,
    },
    {
      key: "where_deliver", label: "Qayergacha",
      render: (val) => <span className="text-sm">{val === "center" ? "Markazgacha" : "Uygacha"}</span>,
    },
    {
  ...
  return (
    <Table data={orders} columns={columns} keyExtractor={(row) => row.id}
      loading={loading} emptyMessage="Buyurtmalar mavjud emas" />
  );
};
