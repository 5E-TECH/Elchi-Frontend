...
import { Eye, EyeOff, Send, Shield, Users, Truck, Store, Building, Calendar, User, ArrowLeft } from 'lucide-react';
...
export const CreateUserForm = memo(() => {
    ...
    const validateForm = () => {
        ...
        else if (formData.password.length < 6) newErrors.password = 'Min 6 ta belgi';
        ...
        if (role === 'marketing') {
            if (!formData.marketName.trim()) newErrors.marketName = 'Nomi kiritilmadi';
    ...
    const labelClasses = "block text-xs font-bold text-slate-500 dark:text-white/60 mb-1.5 ml-1 uppercase tracking-wide";
    ...
    const renderInput = (
        ...
        <div className="space-y-0 relative">
            ...
            {errors[name] && (
                <p className="absolute -bottom-4 right-0 text-[10px] text-red-500 font-medium">{errors[name]}</p>
            )}
        </div>
    );
    ...
    const renderPasswordInput = () => (
        <div className="space-y-0 relative">
            ...
            {errors.password && (
                <p className="absolute -bottom-4 right-0 text-[10px] text-red-500 font-medium">{errors.password}</p>
            )}
        </div>
    );
    ...
    return (
        <div className="w-full h-full flex flex-col overflow-hidden bg-slate-50 dark:bg-background transition-colors duration-300">
            ...
            <div className="flex items-center justify-between px-6 py-4 shrink-0 border-b border-slate-100 dark:border-white/5">
                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 text-slate-500 dark:text-white/70 hover:text-main dark:hover:text-white transition-colors font-medium text-sm px-4 py-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl"
                >
                    <ArrowLeft size={20} />
                    <span>Orqaga</span>
                </button>
            ...
            <div className="flex flex-1 gap-6 overflow-hidden px-6 py-6">
                ...
                <div className="w-72 flex flex-col gap-4 shrink-0">
                    ...
                    <div className="bg-linear-to-br from-main to-indigo-600 rounded-2xl p-5 text-white shadow-lg shadow-main/20">
                        ...
                        <p className="text-white/80 text-xs leading-relaxed">
                            Yangi foydalanuvchi tizimga kiritilgach, unga SMS orqali login va parol yuboriladi.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex-1 bg-white dark:bg-maindark rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-slate-100 dark:border-primarydark/20 flex flex-col overflow-hidden">
                    ...
                    <div className="px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 shrink-0 flex items-center gap-4">
                        <div className={`p-2 rounded-xl bg-linear-to-br text-white shadow-md ${role === 'admin' ? 'from-purple-500 to-indigo-600' :
                            role === 'manager' ? 'from-blue-500 to-cyan-500' :
                                role === 'courier' ? 'from-orange-500 to-amber-500' :
                            ...
                            <div className="space-y-8">
                                ...
                                {(role === 'admin' || role === 'manager') && (
                                    <div className="grid grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                        ...
                                        {renderInput('To\'lov Kuni', 'paymentDay', 'number', 'Sana (1-31)', Calendar)}
                                    </div>
                                )}
                                ...
                                {role === 'courier' && (
                                    <div className="grid grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                        ...
                                        {renderInput('Uyga', 'homeRate', 'number', 'Summa', Building)}
                                        {renderInput('Markazga', 'centerRate', 'number', 'Summa', Store)}
                                    </div>
                                )}

                                {role === 'marketing' && (
                                    <div className="grid grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                        {renderInput('Market Nomi', 'marketName', 'text', 'Nomi', null)}
                                        {renderSelect(
                                            'Yetkazish Turi',
                                            'deliveryType',
                                            [
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-end gap-4 bg-slate-50/50 dark:bg-white/5 shrink-0">
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${role === 'admin' ? 'bg-linear-to-r from-purple-600 to-indigo-600' :
                                    role === 'manager' ? 'bg-linear-to-r from-blue-500 to-cyan-500' :
                                        role === 'courier' ? 'bg-linear-to-r from-orange-500 to-amber-500' :
...
const ShieldIcon = ({ role, size = 24, className }: { role: string, size?: number, className?: string }) => {
    switch (role) {
        case 'admin': return <Shield size={size} className={className} />;
        case 'manager': return <Users size={size} className={className} />;
        case 'courier': return <Truck size={size} className={className} />;
        case 'marketing': return <Store size={size} className={className} />;
